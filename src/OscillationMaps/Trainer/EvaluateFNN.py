...
class EvalModel:
    def __init__(self, datapath='oscillation_maps_extended', verbose=False, plotR=False):
        ...
        self.plot = plotR
        self.dataset_mean, self.dataset_std = [0]*4, [0]*4
    ...
    def eval(self, filepath=''):
        ...
        for mdl in range(4):
            ...
            self.models[mdl].load_state_dict(torch.load(f'{filepath}_NNmodel{mdl}_ind.pt', map_location=self.devices[mdl]))
        ...
        with torch.no_grad():
            ...
            for batch_indices in val_batches:
                ...
                for mdl in range(4):
                    ...
                    if mdl != 3:
                        osc_pred_batch[:, :, mdl, mdl] = np.reshape(output_i.cpu().numpy(), (H, W))
                    else:
                        osc_pred_batch[:, :, 0, 1] = np.reshape(output_i.cpu().numpy(), (H, W))
